...
void read_in_emcmap()
{
  ifstream readmap("/phenix/plhf/roli/combined_electron_photon/run15EMCDeadMap.txt");////"/phenix/plhf/mitran/Analysis/Run14AuAuDiLeptonAnalysis/AnaTrain/test/offline1/AnalysisTrain/DileptonAnalysis/Run14AuAuEmcalDeadMap.txt" /phenix/plhf/tongzhouguo/pi02gg/newRun12.txt
  ///phenix/plhf/roli/combined_electron_photon/run15EMCDeadMap.txt  /phenix/plhf/tongzhouguo/pi02gg/all_209_deadmap.txt
  for (int i = 0; i < 8; ++i)
  {
    for (int j = 0; j < 48; ++j)
    {
                         ...
                         int iter = 0)
{
  ...
  read_in_emcmap();
  TF1* fHagedorn = new TF1("fHagedorn", "2*TMath::Pi()*x*[0]*pow(TMath::Exp(-[1]*x-[2]*x*x)+x/[3],-[4])", 0.2, 40);
  fHagedorn->SetParameters(217.859, 0.503, 0.016, 0.753, 8.283);
  ...
  TString outFile = Form("%s_%d_%d.root", outFile0, system, ((int) shift/10000));
  ...
  for (int iFile = shift; iFile < shift+num; iFile++)
  {
    //const TString inFile0 = Form("%s/test_tree%d/1_tree/sim_trees_%05d.root", inFile, system, iFile);
    const TString inFile0 = Form("%s/test_tree%d/trees/sim_trees_%05d.root", inFile, system, iFile);
