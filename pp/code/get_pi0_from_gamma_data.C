...
const int use_candidate = 0; // 0: use all clusters, 1: use only candidate clusters
...
bool solution(MyElectron* mytrk1, MyElectron* mytrk2, MyPair* mypair, Reconstruction* reco, float zVtx){
...
SystConfig cfgs[NCFG] = {
  {"nominal",       CONV_DZED,     0.3, 0.2, 1, 3.0, 1.00},
  {"conv_solution", CONV_SOLUTION, 0.3, 0.2, 1, 3.0, 1.00},
  {"conv_tight",    CONV_TIGHT,    0.3, 0.2, 1, 3.0, 1.00},
  {"ecore04",       CONV_DZED,     0.4, 0.2, 1, 3.0, 1.00},
  {"ecore05",       CONV_DZED,     0.5, 0.2, 1, 3.0, 1.00},
  {"pte03",         CONV_DZED,     0.3, 0.3, 1, 3.0, 1.00},
  {"pte04",         CONV_DZED,     0.3, 0.4, 1, 3.0, 1.00},
  {"eid0",          CONV_DZED,     0.3, 0.2, 0, 3.0, 1.00},
  {"eid1",          CONV_DZED,     0.3, 0.2, 1, 3.0, 1.00},
  {"eid3",          CONV_DZED,     0.3, 0.2, 3, 3.0, 1.00},
  {"chi2_4",        CONV_DZED,     0.3, 0.2, 1, 4.0, 1.00},
  {"chi2_5",        CONV_DZED,     0.3, 0.2, 1, 5.0, 1.00},
  {"escale099",     CONV_DZED,     0.3, 0.2, 1, 3.0, 0.99},
  {"escale101",     CONV_DZED,     0.3, 0.2, 1, 3.0, 1.01}
};
...
void read_in_emcmap()
{
  ifstream readmap("/phenix/plhf/tongzhouguo/pi02gg/all_209_deadmap.txt");/// /phenix/plhf/tongzhouguo/pi02gg/all_209_deadmap.txt 
  ///phenix/plhf/roli/combined_electron_photon/run15EMCDeadMap.txt
  for (int i = 0; i < 8; ++i)
  {
    for (int j = 0; j < 48; ++j)
    {
...
bool pass_base_track(MyElectron* mytrk)
{
...
double track_pt(MyElectron* mytrk)
{
  return mytrk->GetPtPrime();
}

double track_p(MyElectron* mytrk)
{
...
bool pass_eid(MyElectron* mytrk, int eidType, double ptCut)
{
  if (!mytrk) return false;
...
unsigned int get_track_pass_mask(MyElectron* trk)
{
...
void compute_real_track_masks(std::vector<MyElectron*>& tracks,
...
ClusterInfo make_cluster_info(MyCluster& cl, TF1& f_rand)
{
  ...
  if (ci.mcid != use_candidate ? 0 : 1) return ci;
                             ...
                             const int debug = 1)
{
  ...
  for (int icent = 0; icent < centbin; icent++)
  {
    ...
    //                           0.0, 10.0);
    fHagedorn[icent] = new TF1("fHagedorn", "2*TMath::Pi()*x*[0]*pow(TMath::Exp(-[1]*x-[2]*x*x)+x/[3],-[4])", 0.2, 40);
    fHagedorn[icent]->SetParameters(217.859, 0.503, 0.016, 0.753, 8.283);
  ...
  for (int ievent_A = 0; ievent_A < nevt; ievent_A++)
  {
    ...
    const int ntrack_A = use_candidate ? event->GetNeleccand() : event->GetNtrack();
    const int nclust_A = event->GetNcluster();
    const int cent = event->GetCentrality()*0;///podgon
    const double zVtx_A = event->GetVtxZ() + f_rand.GetRandom() * 0.25; // add random smearing to zVtx
    ...
    // Load track pointers once. Do NOT copy MyElectron objects here:
    // in this embedding class some members can be lost/invalid after by-value copy.
    std::vector<MyElectron*> tracks_A;
    tracks_A.reserve(ntrack_A);
    for (int itrk = 0; itrk < ntrack_A; ++itrk)
    {
      MyElectron* trkp = use_candidate ? event->GetElecCand(itrk) : event->GetEntry(itrk);
    ...
    for (unsigned int itrk = 0; itrk < real_mask_A.size(); ++itrk)
    {
      if (debug)
      {
        ...
        {
          MyElectron* trk_dbg = tracks_A[itrk];
    ...
    for (int ireal_A1 = 0; ireal_A1 < ntracks_loaded; ++ireal_A1)
    {
      ...
      MyElectron* mytrk_A1p = tracks_A[ireal_A1];
      if (!mytrk_A1p) continue;
      MyElectron& mytrk_A1 = *mytrk_A1p;
      ...
      for (int ireal_A2 = ireal_A1 + 1; ireal_A2 < ntracks_loaded; ++ireal_A2)
      {
        ...
        MyElectron* mytrk_A2p = tracks_A[ireal_A2];
        if (!mytrk_A2p) continue;
        MyElectron& mytrk_A2 = *mytrk_A2p;
