...
#include "/phenix/plhf/roli/install/include/PhotonConversionAnalysis/MyEvent.h"
#include "/phenix/plhf/roli/install/include/PhotonConversionAnalysis/Reconstruction.h"
...
const int centbin = 1;
int cent_low[centbin] = {0};
int cent_hi[centbin]  = {55};
double n[centbin] = {0};
...
void read_in_emcmap()
{
  ifstream readmap("/phenix/plhf/tongzhouguo/pi02gg/209_deadmap.txt");////"/phenix/plhf/mitran/Analysis/Run14AuAuDiLeptonAnalysis/AnaTrain/test/offline1/AnalysisTrain/DileptonAnalysis/Run14AuAuEmcalDeadMap.txt"
  for (int i = 0; i < 8; ++i)
  {
    for (int j = 0; j < 48; ++j)
    {
...
{ // start make_TTree
  ...
  for (int ievent_A = firstEvent; ievent_A < lastEvent; ievent_A++)
  {
    ...
    //std::cout<< " Event: " << ievent_A << " ntrack: " << ntrack_A << " nclust: " << nclust_A << " zVtx: " << zVtx_A << " cent: " << cent_A << " bbcq: " << bbcq_A << std::endl;
