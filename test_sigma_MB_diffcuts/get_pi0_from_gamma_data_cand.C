...
void get_pi0_from_gamma_data_cand(const char* inFile = "../tong1.root", const char* outFile = "2d_mass_pt_cand.root", const int system = 0, int ert = 0){ 
  ...
  for (int ievent_A = 0; ievent_A < nevt; ievent_A++){
    ...
    int ntrack_A = event->GetNeleccand();
    ...
    for (int icut = 0; icut < cutbin; ++icut){
      ...
      for (int k1 = 0; k1 < ntrack_A; ++k1){
        if ( !single_cut(event->GetElecCand(k1), icut)) continue; // eid cuts
        all_A[nall_A] = k1;
        ++nall_A;        
        int ghost_flag = 0;
        ...
        for (int k2 = 0; k2 < ntrack_A; ++k2){
          if ( k1==k2 ) continue;
          if ( !single_cut(event->GetElecCand(k2), icut) ) continue;
          // found a RICH ghost pair
          if ( getDcenter((event->GetElecCand(k1))->GetCrkphi(), (event->GetElecCand(k1))->GetCrkz(), (event->GetElecCand(k2))->GetCrkphi(), (event->GetElecCand(k2))->GetCrkz()) < DCENTER_CUT[icut] ) { ghost_flag = 1; break; }
        }
      ...
      for (int ireal_A1 = 0; ireal_A1 < nreal_A; ireal_A1++){ 
        MyTrack mytrk_A1 = *event->GetElecCand(real_A[ireal_A1]);
        ...
        for (int ireal_A2 = ireal_A1+1; ireal_A2 < nreal_A; ireal_A2++){
          MyTrack mytrk_A2 = *event->GetElecCand(real_A[ireal_A2]);
          ...
          for (int iclust_A = 0; iclust_A < nclust_A; ++iclust_A){
            ...
            if(myclust_A.GetMcId()!=0) continue;
