                             ...
                             const int debug = 1)
{
  ...
  TString inFile  = Form("%s/emb_%d.root", inFile0, system+1);
  TString outFile = Form("%s_%d.root", outFile0, system+1);
  ...
  for (int ievent_A = 0; ievent_A < nevt; ievent_A++)
  {
    ...
    const int cent = event->GetCentrality();
    const double zVtx_A = event->GetPreciseZ();
      ...
      for (int ireal_A2 = ireal_A1 + 1; ireal_A2 < ntracks_loaded; ++ireal_A2)
      {
        ...
        for (unsigned int iclust_A = 0; iclust_A < clusters_A.size(); ++iclust_A)
        {
          ...
          for (int icfg = 0; icfg < NCFG; ++icfg)
          {
            ...
            emcPhoton_A.SetPz(e_A*ci.z/r_A);
